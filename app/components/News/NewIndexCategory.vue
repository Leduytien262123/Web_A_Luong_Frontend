...
const fetchData = async () => {
  try {
    const { data: resData } = await restAPI.stores.getBlogCategoryBySlug(path, {
      params: {
        current: currentPage.value,
        pageSize: pageSize.value,
      },
    });
    news.value = resData.value?.data?.posts?.rows || [];
    totalItems.value = resData.value?.data?.posts?.count || 0;
...
news.value = props?.data?.data?.posts?.rows || [];
totalItems.value = props?.data?.data?.posts?.count || 0;
