...
const fetchData = async () => {
  const { data: resDataDetail } = await restAPI.stores.getProductsSearch(
    route.params.slug,
    {
      params: {
...
const productsByCategory = categoriesDetail.value?.rows || [];

const formatPrice = (price) => {
  return price ? new Intl.NumberFormat("vi-VN").format(price) + "đ" : "";
};

const totalItems = ref(categoriesDetail.value?.count);
...
const displayedProducts = computed(() => {
  ...
  return categoriesDetail.value?.rows?.slice(startIndex, endIndex);
});
...
const buildBreadcrumbs = () => {
  const nation_name = [
    ...new Set(productsByCategory.map((cat) => cat.nation_name)),
  ];
  linksBreadcrumb.value.push({
    label: `Kết quả tìm kiếm cho ${nation_name ? nation_name : ""}`,
    to: "",
  });
};
buildBreadcrumbs();
      ...
      <div class="lg:w-[75%] xl:mr-[30px] lg:mr-5">
        ...
        >
          <div v-for="item in displayedProducts" :key="item.name" class="">
            <div
              @click="navigateTo(`/sanpham/${item.slug}`)"
              class="max-w-[310px] 2xl:max-w-full 2xl:max-h-full max-h-[310px] relative cursor-pointer"
            >
              <img
            ...
            <div class="text-lg">
              ...
              >
                {{
                  formatPrice(getCheapestProduct(item.variants)?.price_sale) ||
                  formatPrice(getCheapestProduct(item.variants)?.price)
                }}
              </span>
